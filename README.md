...
- V3 Task 6 已完成核心循环 UI；Task 7、Task 8 自动与人工验收已通过，V3 Task 1–8 已完成。

## 启动项目

使用 `D:\steam\steamapps\common\Godot Engine\godot.windows.opt.tools.64.exe` 打开 `D:\Godot\guild_today\project.godot`，按 `F5` 运行主场景。

Task 8 人工验收已通过。复验入口为 `D:\Godot\guild_today\ui\main\collapsed_mine_guild_main.tscn`，按 `F6` 运行第二张任务场景；它复用同一套正式 UI 与 `GameSession`，只切换正式任务资产。
