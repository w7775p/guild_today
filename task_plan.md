...
- 当前 `project.godot` 主场景为 `ui/main/guild_main.tscn`；正式 `GameSession` 与核心循环 UI 已建立。
...
本机 Godot 4.7.2 可执行文件：`D:\steam\steamapps\common\Godot Engine\godot.windows.opt.tools.64.exe`。后续命令直接使用该绝对路径，不依赖 PATH。
...
- 六名角色资源全部加载。
- 五个结果各有一条确定性派遣命中；若正式角色能力值无法覆盖同一结果区间，允许在专项测试中临时创建普通 `CharacterAsset` 作为数值覆盖派遣成员。
- 两日等待前后生命周期正确。
- 五个结果结算后状态与报告正确。
- 重复结算、重复占用、到期前结算被拒绝。
...
- 六名正式角色的全部单人及双人派遣共 21 种组合中，“带伤救回”命中数为 0；其条件要求调查能力至少 5 且战斗能力为 4–5。
- 当前角色组合只覆盖：完整成功 11 种、延迟成功 4 种、查明位置 3 种、搜索失败 3 种。
- Jackie 已决定保留正式角色与结果条件；当同一结果仅因能力数值区间没有正式命中时，测试中临时创建普通 `CharacterAsset` 覆盖该结果，该情况按数值覆盖缺口验收，不判定为角色设计缺陷。
- 临时测试角色只验证条件、两日生命周期、结算与报告链，不进入正式角色池，不改变正式运行路径，也不评价角色设计。
- 当前“带伤救回”使用战斗 5、调查 5 的临时测试角色覆盖；正式运行路径继续只加载六名正式角色。
...
当前下一步：V3 Task 1–8 已完成并通过验收；Demo Readiness 深度审计修复与二次验证完成后停止扩展，重新制定 V4 计划。

最近完成更新：Task 8 已通过第二张正式任务的 8 结果路线与人工验收；随后完成 Demo Readiness 深度审计第一轮修复及修改后复审，当前结果、结算、派遣、路径和测试退出码均需以 `findings.md` 的已验证清单为准。

备注：当前机器的 Godot 运行环境会输出日志目录和根证书告警；“正常人工场景零 ERROR”保留为原验收条件，环境限制下标记 **UNVERIFIED**。
