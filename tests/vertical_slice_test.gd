...
func _ready() -> void:
	...
	if passed:
		print("Vertical slice test success: checks=all")
	else:
		push_error("Vertical slice test failed: checks=%d" % _failed_checks)
...
func _validate_character_asset() -> void:
	...
	_check(character != null, "CharacterAsset fixture 加载失败")
	_check(String(character.id) == "test_character_a", "CharacterAsset ID 不正确")
	_check(character.name == "角色A", "CharacterAsset 名称不正确")
	_check(character.battle == 5, "CharacterAsset battle 不正确")
	_check(character.investigation == 10, "CharacterAsset investigation 不正确")
	_check(character.negotiation == 7, "CharacterAsset negotiation 不正确")
...
func _validate_task_asset() -> void:
	...
	_check(task != null, "TaskAsset fixture 加载失败")
	_check(String(task.id) == "test_abandoned_hospital", "TaskAsset ID 不正确")
	_check(task.title == "废弃医院调查", "TaskAsset 标题不正确")
	_check(task.description == "调查废弃医院中的异常情况。", "TaskAsset 描述不正确")
	_check(task.repeatable == false, "TaskAsset repeatable 不正确")
	_check(task.result_group is ResultGroupAsset, "TaskAsset 应绑定 ResultGroupAsset")
	_check(task.event_reference == null, "TaskAsset 不应提前绑定 EventAsset")
...
func _validate_ability_condition() -> void:
	...
	_check(threshold_10 != null, "AbilityCondition 阈值 10 fixture 加载失败")
	_check(threshold_13 != null, "AbilityCondition 阈值 13 fixture 加载失败")
	_check(threshold_10.value == 10, "AbilityCondition 阈值 10 读取错误")
	_check(threshold_13.value == 13, "AbilityCondition 阈值 13 读取错误")
	_check(threshold_10.is_met(12), "调查能力 12 应满足阈值 10")
	_check(not threshold_13.is_met(12), "调查能力 12 不应满足阈值 13")
...
func _validate_effect_resources() -> void:
	...
	_check(gold_effect != null, "GoldEffect fixture 加载失败")
	_check(reputation_effect != null, "ReputationEffect fixture 加载失败")
	_check(gold_effect is EffectResource, "GoldEffect 必须继承 EffectResource")
	_check(reputation_effect is EffectResource, "ReputationEffect 必须继承 EffectResource")
	_check(gold_effect.amount == 100, "GoldEffect amount 不正确")
	_check(reputation_effect.amount == 5, "ReputationEffect amount 不正确")
...
func _validate_result_asset() -> void:
	...
	_check(result != null, "ResultAsset fixture 加载失败")
	_check(String(result.id) == "test_abandoned_hospital_success", "ResultAsset ID 不正确")
	_check(result.conditions.size() == 1, "ResultAsset 条件数量不正确")
	_check(result.effects.size() == 2, "ResultAsset 效果数量不正确")
	_check(result.conditions[0] is AbilityCondition, "ResultAsset 条件类型不正确")
	_check((result.conditions[0] as AbilityCondition).value == 10, "ResultAsset 调查阈值不正确")
	_check(result.effects[0] is GoldEffect, "ResultAsset 金币效果类型不正确")
	_check((result.effects[0] as GoldEffect).amount == 100, "ResultAsset 金币效果值不正确")
	_check(result.effects[1] is ReputationEffect, "ResultAsset 声望效果类型不正确")
	_check((result.effects[1] as ReputationEffect).amount == 5, "ResultAsset 声望效果值不正确")
	_check(result.report_text == "废弃医院调查完成", "ResultAsset 报告文本不正确")
...
func _validate_result_group_asset() -> void:
	...
	_check(result_group != null, "ResultGroupAsset fixture 加载失败")
	_check(String(result_group.id) == "test_abandoned_hospital_results", "ResultGroupAsset ID 不正确")
	_check(result_group.results.size() == 2, "ResultGroupAsset 结果数量不正确")
	_check(String(result_group.results[0].id) == "test_abandoned_hospital_success", "成功结果 ID 不正确")
	_check(String(result_group.results[1].id) == "test_abandoned_hospital_failure", "失败结果 ID 不正确")
	_check(result_group.results[0].conditions[0] is AbilityCondition, "成功结果条件类型不正确")
	_check(result_group.results[1].conditions[0] is AbilityBelowCondition, "失败结果条件类型不正确")
	...
	_check(failure_condition.value == 10, "失败结果阈值不正确")
	_check(not success_condition.is_met(9) and failure_condition.is_met(9), "调查能力 9 应只满足失败条件")
	_check(success_condition.is_met(10) and not failure_condition.is_met(10), "调查能力 10 应只满足成功条件")
...
func _validate_task_instance() -> void:
	...
	_check(first_instance.instance_id != second_instance.instance_id, "TaskInstance 实例 ID 必须不同")
	_check(first_instance.task_id == second_instance.task_id, "两个实例应引用同一静态任务 ID")
	_check(first_instance.lifecycle_state == &"IN_PROGRESS", "第一个实例生命周期事实不正确")
	_check(second_instance.lifecycle_state == &"PUBLISHED", "第二个实例生命周期事实被污染")
	_check(first_instance.final_result_id == &"test_abandoned_hospital_success", "第一个实例最终结果 ID 不正确")
	_check(second_instance.final_result_id.is_empty(), "第二个实例最终结果 ID 被污染")
	_check(first_instance.runtime_progress[&"investigation"] == 2, "第一个实例进度不正确")
	_check(second_instance.runtime_progress[&"investigation"] == 3, "第二个实例进度被污染")
...
func _validate_dispatch_instance() -> void:
	...
	_check(character_a != null, "DispatchInstance 角色 A 加载失败")
	...
	_check(dispatch.dispatch_instance_id == &"test_dispatch_instance_001", "DispatchInstance ID 不正确")
	_check(dispatch.task_instance_id == &"test_task_instance_001", "DispatchInstance 任务实例 ID 不正确")
	_check(dispatch.character_refs.size() == 3, "DispatchInstance 应保存三个角色引用")
	_check(dispatch.character_refs[0] == character_a, "DispatchInstance 角色 A 引用不正确")
	_check(dispatch.character_refs[1] == character_b, "DispatchInstance 角色 B 引用不正确")
	_check(dispatch.character_refs[2] == character_c, "DispatchInstance 角色 C 引用不正确")
	_check(dispatch.status == &"ACTIVE", "DispatchInstance 状态不正确")
	_check(not "party_member_ids" in TaskInstance.new(), "TaskInstance 不得保存可变派遣成员副本")
...
func _validate_result_instance() -> void:
	...
	_check(result_asset != null, "ResultInstance 所需 ResultAsset 加载失败")
	...
	_check(result_instance.result_asset_id == &"test_abandoned_hospital_success", "ResultInstance 结果资产 ID 不正确")
	_check(result_instance.dispatch_instance_id == &"test_dispatch_instance_001", "ResultInstance 派遣实例 ID 不正确")
	_check(result_instance.resolved_effects.size() == 2, "ResultInstance 应保存两个已确定效果")
	_check(result_instance.resolved_effects[0] == gold_effect, "ResultInstance 金币效果引用不正确")
	_check(result_instance.resolved_effects[1] == reputation_effect, "ResultInstance 声望效果引用不正确")
	result_instance.resolved_effects.reverse()
	_check(result_asset.effects[0] == gold_effect, "ResultInstance 数组变化不应改写 ResultAsset")
	_check(result_asset.effects[1] == reputation_effect, "ResultAsset 效果顺序被运行记录污染")
	result_instance.resolved_effects.reverse()
...
func _validate_task_system() -> void:
	...
	_check(task_asset != null, "TaskSystem 所需 TaskAsset 加载失败")
	...
	_check(first_instance != null and second_instance != null, "TaskSystem 创建任务实例失败")
	_check(first_instance.instance_id != second_instance.instance_id, "TaskSystem 生成了重复实例 ID")
	_check(first_instance.task_id == task_asset.id, "第一个 TaskInstance 的任务 ID 不正确")
	_check(second_instance.task_id == task_asset.id, "第二个 TaskInstance 的任务 ID 不正确")
	_check(first_instance.lifecycle_state == &"PUBLISHED", "第一个 TaskInstance 初始状态不正确")
	_check(second_instance.lifecycle_state == &"PUBLISHED", "第二个 TaskInstance 初始状态不正确")
	_check(task_system.get_task_instance(first_instance.instance_id) == first_instance, "TaskSystem 未保存第一个实例")
	_check(task_system.get_task_instance(second_instance.instance_id) == second_instance, "TaskSystem 未保存第二个实例")
	_check(task_system.start_task_instance(first_instance.instance_id), "TaskSystem 启动第一个实例失败")
	_check(task_system.record_final_result(first_instance.instance_id, &"test_abandoned_hospital_success"), "TaskSystem 记录最终结果失败")
	_check(first_instance.final_result_id == &"test_abandoned_hospital_success", "TaskSystem 最终结果写入不正确")
	_check(second_instance.final_result_id.is_empty(), "TaskSystem 污染了另一个任务实例")
...
func _validate_dispatch_system() -> void:
	var character: CharacterAsset = load("res://tests/fixtures/character_a.tres") as CharacterAsset
	_check(character != null, "DispatchSystem 所需角色加载失败")
	...
	_check(first_dispatch != null, "DispatchSystem 第一次派遣创建失败")
	_check(first_dispatch.status == &"ACTIVE", "第一次派遣应为 ACTIVE")
	_check(dispatch_system.get_dispatch_instance(first_dispatch.dispatch_instance_id) == first_dispatch, "DispatchSystem 未保存第一次派遣")
	_check(dispatch_system.is_character_occupied(character.id), "ACTIVE 派遣应占用角色")

	var blocked_dispatch := dispatch_system.create_dispatch(&"test_task_instance_002", party)
	_check(blocked_dispatch == null, "同一角色不得同时进入第二个 ACTIVE 派遣")
	_check(dispatch_system.end_dispatch(first_dispatch.dispatch_instance_id), "DispatchSystem 结束第一次派遣失败")
	_check(first_dispatch.status == &"ENDED", "第一次派遣应变为 ENDED")
	_check(not dispatch_system.is_character_occupied(character.id), "ENDED 派遣不应继续占用角色")

	var second_dispatch := dispatch_system.create_dispatch(&"test_task_instance_002", party)
	_check(second_dispatch != null, "角色释放后应允许再次派遣")
	_check(second_dispatch.status == &"ACTIVE", "第二次派遣应为 ACTIVE")
	_check(second_dispatch.dispatch_instance_id != first_dispatch.dispatch_instance_id, "两次派遣必须使用不同实例 ID")
...
func _validate_task_resolution_system() -> void:
	...
	_check(success_result_id == &"test_abandoned_hospital_success", "调查能力达标时应选择成功结果")
	...
	_check(failure_result_id == &"test_abandoned_hospital_failure", "调查能力不足时应选择失败结果")
	...
	_check(resolution_system.resolve_result(successful_dispatch, zero_match_group).is_empty(), "零命中必须明确失败")
	...
	_check(resolution_system.resolve_result(successful_dispatch, multiple_match_group).is_empty(), "多命中必须明确失败")
...
func _validate_guild_state() -> void:
	...
	_check(guild_state.has_state(GuildState.GOLD_STATE_ID), "GuildState 缺少金币状态")
	_check(guild_state.has_state(GuildState.TOTAL_REPUTATION_STATE_ID), "GuildState 缺少总声望状态")
	_check(guild_state.get_value(GuildState.GOLD_STATE_ID) == 0, "GuildState 金币初始值应为 0")
	_check(guild_state.get_value(GuildState.TOTAL_REPUTATION_STATE_ID) == 0, "GuildState 总声望初始值应为 0")

	# 先验证设置与恢复入口，再从恢复后的 0 执行本 Task 的增量验收。
	guild_state.set_value(GuildState.GOLD_STATE_ID, 25)
	_check(guild_state.get_value(GuildState.GOLD_STATE_ID) == 25, "GuildState 金币设置结果不正确")
	guild_state.restore_value(GuildState.GOLD_STATE_ID, 0)
	_check(guild_state.get_value(GuildState.GOLD_STATE_ID) == 0, "GuildState 金币恢复结果不正确")
	guild_state.add_value(GuildState.GOLD_STATE_ID, 100)
	guild_state.add_value(GuildState.TOTAL_REPUTATION_STATE_ID, 5)
	_check(guild_state.get_value(GuildState.GOLD_STATE_ID) == 100, "GuildState 金币增加结果不正确")
	_check(guild_state.get_value(GuildState.TOTAL_REPUTATION_STATE_ID) == 5, "GuildState 总声望增加结果不正确")
...
func _validate_result_settlement_system() -> void:
	var result_asset: ResultAsset = load("res://tests/fixtures/test_abandoned_hospital_success.tres") as ResultAsset
	_check(result_asset != null, "ResultSettlementSystem 所需成功结果加载失败")
	var result_instance := ResultInstance.new()
	result_instance.result_asset_id = result_asset.id
	...
	dispatch_instance.dispatch_instance_id = result_instance.dispatch_instance_id
	dispatch_instance.task_instance_id = &"test_settlement_task_001"
	dispatch_instance.status = &"ACTIVE"
	add_child(guild_state)
	add_child(settlement_system)
	_check(settlement_system.settle_result(result_instance, guild_state, null, null, null, null, null, dispatch_instance, -1, result_asset), "ResultSettlementSystem 首次结算失败")
	_check(guild_state.get_value(GuildState.GOLD_STATE_ID) == 100, "结算后金币应为 100")
	_check(guild_state.get_value(GuildState.TOTAL_REPUTATION_STATE_ID) == 5, "结算后总声望应为 5")

	_check(not settlement_system.settle_result(result_instance, guild_state, null, null, null, null, null, dispatch_instance, -1, result_asset), "ResultSettlementSystem 必须拒绝重复结算")
	_check(guild_state.get_value(GuildState.GOLD_STATE_ID) == 100, "重复结算不应再次增加金币")
	_check(guild_state.get_value(GuildState.TOTAL_REPUTATION_STATE_ID) == 5, "重复结算不应再次增加总声望")
...
func _validate_report_intel_system() -> void:
	var result_asset: ResultAsset = load("res://tests/fixtures/test_abandoned_hospital_success.tres") as ResultAsset
	_check(result_asset != null, "ReportIntelSystem 所需成功结果加载失败")
	var result_instance := ResultInstance.new()
	result_instance.result_asset_id = result_asset.id
	...
	var dispatch_instance := DispatchInstance.new()
	dispatch_instance.dispatch_instance_id = result_instance.dispatch_instance_id
	dispatch_instance.task_instance_id = &"test_report_task_001"
	dispatch_instance.status = &"ACTIVE"
	add_child(guild_state)
	add_child(settlement_system)
	add_child(report_system)
	_check(settlement_system.settle_result(result_instance, guild_state, null, null, null, null, null, dispatch_instance, -1, result_asset), "报告生成前的效果结算失败")
